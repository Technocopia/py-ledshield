...
def rangeSplit(colorLower, colorUpper):
    ...
    if hLower < 0 or hUpper > 180:
        if hLower < 0:
            rangeList.append((np.array([180+hLower, sLower, vLower]),np.array([180, sUpper, vUpper])))
            rangeList.append((np.array([0, sLower, vLower]),np.array([hUpper, sUpper, vUpper])))
        if hUpper > 180:
            rangeList.append((np.array([hLower, sLower, vLower]),np.array([180, sUpper, vUpper])))
            rangeList.append((np.array([0, sLower, vLower]),np.array([180-hUpper, sUpper, vUpper])))
    else:
...
class TrackingColor(object):
    ...
    def __init__(self, bgr, threshold=15,saturationRange=(150,255),valueRange=(150,255)):
        self.bgr = bgr
    ...
    def rangeList(self):
        if self._rangeList is None:
            colorLower, colorUpper = bgrToHsvRange(self.bgr, threshold=self.threshold, saturationRange=self.saturationRange, valueRange=self.valueRange)
...
class Tracker(object):
    ...
    def update(self, hsvFrame):
        ...
        self.contours,hierarchy = cv2.findContours(self.colorMask.copy(), cv2.RETR_EXTERNAL,
...
class AirDraw(object):
    ...
    def loop(self):
        ...
        for tracker in self.trackers:
            trackingColor = tracker.trackingColor
            tracker.update(hsv)
            ...
            if tracker.x and tracker.y and tracker.radius:
                cv2.circle(frame, (int(tracker.x), int(tracker.y)), int(tracker.radius), tracker.trackingColor.bgr, 2)
                if self.showCanvas:
                    cv2.circle(self.canvas, (int(tracker.x), int(tracker.y)), int(tracker.radius), tracker.trackingColor.bgr, -1)
